...
{if(this[i]==value)
{return true;}}
return false;};FreiChat.search_members=function(user_data,user_array){var userdata=[];var search_value=$jn.trim($jn("#frei_member_search_input").val());var user_arr_len=user_array.length;var u=0;var i=0;var curr_user;if(search_value!=""){for(u=0;u<user_arr_len;u++){curr_user=user_array[u].toLowerCase();if(curr_user.indexOf(search_value)!=-1){userdata[i]=user_data[u];i++;}}}else{userdata=user_data}
if(user_data==null||user_data==''){userdata=null;alert("users dont exist");}
else{if(userdata.length>0){userdata=userdata.join(" ");}else{userdata=freidefines.TRANS.no_search_results;}}
return userdata;}
...
{FreiChat.RequestCompleted_get_members=false;FreiChat.set_custom_mesg();var in_room=FreiChat.in_room;if(!$jn('#dc-slick-9').hasClass('active')&&FreiChat.first!=false){in_room=-1;}
$jn.getJSON(freidefines.GEN.url+"server/freichat.php?freimode=getmembers",{xhash:freidefines.xhash,id:freidefines.GEN.getid,first:FreiChat.first,time:FreiChat.time,chatroom_mesg_time:FreiChat.chatroom_mesg_time,'clrchtids[]':[FreiChat.clrchtids],custom_mesg:FreiChat.custom_mesg,long_poll:FreiChat.long_poll,in_room:in_room},function(data){if(data==null){FreiChat.divfrei.html(freidefines.TRANS.ban_mesg);clearTimeout(FreiChat.c);return;}
...
var userdata=null;if(data.userdata!=null&&data.userdata!=''){userdata=FreiChat.search_members(data.userdata,data.user_array);}
FreiChat.userdata=data.userdata;FreiChat.user_array=data.user_array;if(userdata==null)
{userdata=freidefines.nolinemesg;FreiChat.divfrei.html(userdata);}
else
{FreiChat.divfrei.html(userdata)}
...
{FreiChat.freistatus=1;FreiChat.min_max_freichat('max');FreiChat.ursimg.attr('src',FreiChat.make_url(freidefines.onlineimg));if(FreiChat.mainchat.is(":visible")==false)
...
{FreiChat.ursimg.attr('src',FreiChat.make_url(freidefines.invisibleimg));FreiChat.min_max_freichat('min');FreiChat.freistatus=2;}
else if(opt=="goBusy")
{FreiChat.ursimg.attr('src',FreiChat.make_url(freidefines.busyimg));FreiChat.min_max_freichat('min');FreiChat.freistatus=3;}
else if(opt=="goTempBusy")
{FreiChat.min_max_freichat('min');FreiChat.ursimg.attr('src',FreiChat.make_url(freidefines.busyimg));FreiChat.freistatus=4;}
else
...
{pluginhtml+='<span id="videosend'+id+'"><a href="javascript:void(0)" onClick="FreiChat.sendvideo(\''+user+'\',\''+id+'\',1)"><img id="mail_'+id+'" src="'+FreiChat.make_url(freidefines.videoimg)+'" title='+freidefines.titles_videochat+' alt="upload" /> </a></span>';}}
...
{FreiChat.chatroom.dcSlick({location:'left',classWrapper:'frei_chatroom',classContent:'frei_chatroom-content',align:'left',offset:'50px',speed:'slow',classTab:'frei_tab',tabText:freidefines.TRANS.chatroom_label,autoClose:true});var selected_chatroom=Get_Cookie('selected_chatroom');if(selected_chatroom==null){selected_chatroom=1;}
FreiChat.in_room=selected_chatroom;FreiChat.my_name='<div></div><div id="frei_userlist" class="frei_userlist" onmouseover="$jn(this).addClass(\'frei_userlist_onhover\');" onmouseout="$jn(this).removeClass(\'frei_userlist_onhover\');"><span class="freichat_userscontentname">'+freidefines.GEN.fromname+'</span></div>';$jn('#frei_userpanel').html(FreiChat.my_name);FreiChat.set_smileys();}
...
/* Updated 24 October 2012 1:55 pm FreiChatX  V.7.6 */